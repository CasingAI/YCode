...
- 浏览器 / Web 端必须能收起并重新展开侧栏：顶部浮层的侧栏切换按钮在所有环境渲染，只有 Windows/Linux 桌面（自定义标题栏）用 logo 代替图标。侧栏自身不带折叠控件，且窗口 resize 触发的自动收起策略会在窄视口收紧侧栏，浮层按钮是唯一入口。
- 顶部浮层只保留侧栏切换、新建任务、更新状态三个入口，不再渲染任务前进/后退按钮：远端浏览器窗口窄，这两个按钮与主操作拥挤，任务切换由侧栏任务列表和快捷键承担。
- 浏览器与远端设备不在窗口标题栏内，没有窗口控件安全区，浮层左侧必须留出与 Windows/Linux 桌面一致的内边距并按浮层高度垂直居中，否则侧栏切换按钮会紧贴窗口左上角。
- 状态分两根轴：
...
9. 远端窄视口顶部左侧的切换按钮可用：收起侧栏后仍能靠它重新展开，且按钮不紧贴窗口左上角（保留与桌面端一致的左边距）。
