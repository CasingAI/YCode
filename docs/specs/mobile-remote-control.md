...
- 浏览器 / Web 端必须能收起并重新展开侧栏：侧栏切换入口在所有环境存在。桌面由顶部浮层承担（Windows/Linux 桌面用 logo 代替图标）；网页版（`isDesktop=false`）**侧栏展开时同样由浮层承担、与桌面同源，只有侧栏收起时才把入口移进各视图自己的顶部区域**。判定只看侧栏是否收起，与窗口宽度无关。侧栏自身不带折叠控件，且窗口 resize 触发的自动收起策略会在窄视口收紧侧栏，这个入口是唯一的折叠/展开开关。
...
- 网页版侧栏收起时入口的落点与各视图既有 header 对齐，不另立新 header；入口组自带 `gap` 与标题留出间距，不被标题或按钮相互挤压：
  ...
  - automations / plugin-store 参考桌面客户端的 `h-12` 面包屑带（`AutomationsMainBreadcrumbFrame`），网页版此时渲染同一条带子，入口在带内面包屑左侧。
...
9. 网页版顶部入口跟随侧栏状态：**侧栏展开时（无论窗口多宽）**入口留在左上浮层原位、与桌面观感一致，各页面不出现多余带子、内容不被按钮压住；点浮层里的切换按钮收起侧栏后，入口移到当前视图自己的顶部区域（Chat 是与标题同行的左侧、automations 是与客户端同款的 h-12 面包屑带），按钮之间及与标题之间留有正常间距、不叠放，浮层同时消失；再次点该按钮可在原位置重新展开侧栏并恢复浮层。
