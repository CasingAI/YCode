...
9. 网页版顶部入口跟随侧栏状态：**侧栏展开时（无论窗口多宽）**入口留在左上浮层原位、与桌面观感一致，Chat 不出现第二层带子；点浮层里的切换按钮收起侧栏后，入口移到当前视图自己的顶部区域（Chat 是与标题同行的左侧、automations/plugin-store 是与客户端同款 h-12 面包屑带左侧预留的槽位），按钮之间及与标题之间留有正常间距、不叠放，浮层同时消失；再次点该按钮可在原位置重新展开侧栏并恢复浮层。automations / plugin-store 的带子在网页版常驻（与客户端一致），收起/展开只改变槽位里有没有入口，内容与面包屑的位置不发生变化。
