...
- **文件内容 = 计划正文本身**，不加 YAML frontmatter（与 `writing-plans` skill 的约定一致）；标题消费方按首个 H1/首个非空行推导（与 UI `getPlanDirectoryTitle` 同规则）。
- **多计划**：每次 `ExitPlanMode` 写一个新文件，从不覆盖旧文件。
- **压缩回注**：压缩时读会话计划目录中最新一份，作为 `plan_file_reference` 系统提醒注入（source 与生命周期不变，只换读取逻辑）。
- **`ListPlans` 工具**：只读、免审批、always-on；一次调用返回本会话全部计划（标题、文件路径、时间、是否最新）与**最新一份的全文**；非最新计划只给路径，模型自行用 Read 读取。任何模式下可用（压缩、换回合后都靠它找计划）。
...
- `ListPlans`：契约在 `@zcode/contracts`（`tools/session-plans.ts`），handler 在 `core/src/tool/handlers/list-plans.ts`，注册进 `builtInTools`。输出含 `plans[]`（`planId`、`path`、`title`、`createdAt`、`isLatest`）与 `latest`（含 `content`），列表按时间升序、最新在末尾。
...
6. `pnpm typecheck`、`pnpm lint`、`pnpm architecture:check --changed` 通过；`core` 包测试通过。
