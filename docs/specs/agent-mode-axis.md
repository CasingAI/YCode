# Spec: Agent 模式轴（计划 / 只读 / 完全访问）

## 目标

把会话的权限约束收敛成一根三值轴：`plan`（计划模式）、`readonly`（只读模式）、`yolo`（完全访问），默认 `yolo`。同一个值同时决定两件事：权限层怎么判，以及模型收到什么样的模式提醒。composer 上只留一个三选单选组，不再有复选项、说明行与标记药丸。
...
- **三档语义。** 计划模式 = 只读 + 4 阶段计划工作流 + 必须以 `ExitPlanMode` 结束回合；只读模式 = 只读，没有 in-band 退出工具；完全访问 = 权限层不弹窗。
...
- **每个用户回合都注入模式提醒。** 权限轴只有三档，短标记成本可忽略；`TURNS_BETWEEN_ATTACHMENTS = 1`。完全访问也有提醒，让模型知道自己没有被权限层拦着。历史提醒按档位分别计数（匹配正文里的稳定前缀），避免一档的提醒吃掉另一档的计数。
...
- **runtime**：`setExecutionState`（命令路径）与 `applyRuntimeExecutionState`（工具路径）是仅有的两个写入点。
